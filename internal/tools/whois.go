...
func parseRdapResponse(domain string, rdap map[string]interface{}) WhoisResult {
	...
	// Validate RDAP response contains the correct domain
	// If RDAP returns a TLD record, ldhName will be the TLD, not the domain
	if ldhName, ok := rdap["ldhName"].(string); ok {
		ldhNameLower := strings.ToLower(ldhName)
		domainLower := strings.ToLower(domain)
		tldLower := strings.ToLower(extractTLD(domain))
		// Check if ldhName matches the domain or is just the TLD
		if ldhNameLower != domainLower && ldhNameLower != "."+tldLower {
			// ldhName is something else - might be a TLD record or wrong domain
			result.Error = fmt.Sprintf("RDAP returned record for %s instead of %s", ldhName, domain)
			return result
		}
	}
...
// discoverTLDInfo discovers the WHOIS and RDAP servers for any TLD.
// It first checks knownWhoisServers/knownRdapServers for fast lookup,
// then falls back to IANA queries for unknown TLDs.
func discoverTLDInfo(ctx context.Context, tld string) tldInfo {
	info := tldInfo{}
	tld = strings.ToLower(tld)

	// Step 1: Check knownWhoisServers first (fast path, no network call)
	if server, ok := knownWhoisServers[tld]; ok {
		info.whoisServer = server
	}

	// Step 2: Check knownRdapServers (fast path, no network call)
	if server, ok := knownRdapServers[tld]; ok {
		info.rdapURL = server
	}

	// Step 3: If not found in known maps, query IANA for WHOIS server
	if info.whoisServer == "" {
...
func fetchIANAHTMLPage(ctx context.Context, tld string) string {
	...
	if matches := remarksPattern.FindStringSubmatch(html); len(matches) > 1 {
		...
		// If conversion failed, the URL is not a valid RDAP endpoint - don't return it
		// This prevents using homepage URLs like http://www.verisigninc.com
	}
...
func convertToRdapURL(regURL string) string {
	...
	normalized := strings.TrimSuffix(strings.ToLower(regURL), "/")
	...
	if rdapURL, ok := registryToRdap[normalized]; ok {
		return rdapURL
	}

	// Try with trailing slash variations
	urls := []string{normalized, strings.TrimSuffix(normalized, "/"), normalized + "/"}
	for _, u := range urls {
...
func isIanaTLDRecord(response, domain string) bool {
	...
	// Strong domain record indicators - if these are present, it's definitely a domain record
	domainRecordIndicators := []string{
		"registry domain id:",
		"creation date:",
		"registry expiry date:",
		"registrar:",
	}
	domainIndicatorCount := 0
	for _, indicator := range domainRecordIndicators {
		if strings.Contains(responseLower, indicator) {
			domainIndicatorCount++
		}
	}

	// If we have multiple domain record indicators, it's a valid domain record
	if domainIndicatorCount >= 2 {
		return false
	}

	// Check if the domain name appears in the response
	domainAppears := strings.Contains(responseLower, domainLower)

	// TLD records typically contain references to the TLD but not the full domain
	// or contain "TLD" in the response
	tldAppears := strings.Contains(responseLower, "."+tldLower)
...
func CheckWhois(domain string, refresh bool) WhoisResult {
	...
	// Prefer known RDAP servers over discovered ones - discovered URLs from IANA HTML
	// pages are often homepage URLs, not actual RDAP endpoints
	rdapServer := getRdapServer(tld)
	if rdapServer == "" {
		// Fallback to discovered RDAP URL if known server not found
	...
	if !whoisSucceeded && rdapServer != "" {
		...
		if rdapData, rdapErr := queryRdap(ctx, rdapServer, domain); rdapErr == nil {
			...
			// RDAP returned data but parsing had issues - return RDAP error
			result.Error = rdapResult.Error
			result.Duration = rdapResult.Duration
			return result
		}
		// RDAP query failed - record this error and continue to return proper error
		// Don't fall through to WHOIS error if RDAP is available
	}

	// If we get here, either WHOIS succeeded or RDAP was not available
	if err != nil {
