...
import { Lock, Loader2, CheckCircle2, XCircle, AlertTriangle, Mail } from 'lucide-react'
...
export default function TLSRPTCheck() {
  ...
  const cached = getLastResult('tlsrpt')
  ...
  const run = async () => {
    ...
    try {
      const res = await api.tlsrpt(input.trim())
      setData(res.data)
      setLastResult('tlsrpt', input.trim(), res.data)
      addHistory(input.trim(), 'TLS-RPT Check')
      toast('TLS-RPT check complete', 'success')
  ...
  return (
    <div>
      <PageHeader icon={Lock} title="TLS-RPT Check" subtitle="TLS failure reporting endpoint for SMTP" gradient="from-emerald-500/20 to-teal-500/10" badge="new">
        ...
        <div className="space-y-4 animate-fade-in-up">
          <div className="flex items-center justify-between p-5 rounded-2xl bg-surface border border-border">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-base font-semibold">{data.valid ? 'TLS-RPT Configured' : 'TLS-RPT Missing'}</div>
                <div className="text-xs text-text-secondary font-mono">{data.domain} · fetched in {data.duration_ms}ms</div>
              </div>
            </div>
            <ExportButton data={data} filename={`tlsrpt-${data.domain}`} label="Export" />
          </div>

          {data.version && (
            <div className="p-4 rounded-2xl bg-surface border border-border">
              <div className="text-[10px] font-semibold uppercase tracking-wider text-text-muted mb-1">Version</div>
              <div className="text-xl font-bold font-mono">{data.version}</div>
            </div>
          )}

          {data.rua && data.rua.length > 0 && (
            <div className="p-4 rounded-2xl bg-surface border border-border">
              <div className="flex items-center gap-2 mb-3">
                <Mail className="w-4 h-4 text-text-secondary" />
                <h3 className="text-sm font-semibold">Reporting URIs (rua)</h3>
              </div>
              <div className="space-y-1">
                {data.rua.map((r: string, i: number) => (
                  <div key={i} className="px-3 py-2 rounded-lg bg-surface-2 font-mono text-xs break-all">{r}</div>
                ))}
              </div>
            </div>
          )}
